...
bool LambertianScatter(in Lambertian lambertian, in Ray incident, in HitRecord hitRecord, out Ray scattered, out vec3 attenuation){
	...
	scattered.direction = hitRecord.normal + random_in_unit_sphere();//origin和direction都改变
...
bool WorldHit(World world, Ray ray, float t_min, float t_max, inout HitRecord rec){//重要函数！
    HitRecord tempRec;
...
void main(){
	...
	for(int i = 0; i < ns; i++){
		...
		col += WorldTrace(world, ray, 50);//简单求和平均，最大深度50
	}
	col /= ns;
